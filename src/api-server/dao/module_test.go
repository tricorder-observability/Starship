...
func TestModule(t *testing.T) {
	...
	module := &ModuleGORM{
		ID:                 id,
		DesiredStatus:      int(pb.DeploymentStatus_CREATED),
		Name:               "TestModule",
		Wasm:               []byte("WasmUid"),
	...
	// if module.Name != TestModule, code save error
	...
	// test code.Status
	if module.DesiredStatus != int(pb.DeploymentStatus_CREATED) {
		t.Errorf("query module status error, module.Status != DeploymentStatus_CREATED ")
	}

	// test update module status
	err = moduleDao.UpdateStatusByID(module.ID, int(pb.DeploymentStatus_TO_BE_DEPLOYED))
	if err != nil {
	...
	// check module status
	if module.DesiredStatus != int(pb.DeploymentStatus_TO_BE_DEPLOYED) {
		t.Errorf("change module status by ID error: not change module status")
	}
	// get module list *
	list, err := moduleDao.List("*")
	if err != nil {
	...
	// get module list default
	list, err = moduleDao.List("id", "name")
	if err != nil {
		t.Errorf("query module list default error: %v", err)
	}
	if len(list) == 0 {
