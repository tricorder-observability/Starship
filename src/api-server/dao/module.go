...
func (g *ModuleDao) UpdateStatusByID(id string, desiredStatus int) error {
	result := g.Client.Engine.Model(&ModuleGORM{ID: id}).Select("desired_status").Updates(ModuleGORM{DesiredStatus: desiredStatus})
	return result.Error
}
...
func (g *ModuleDao) List(query ...string) ([]ModuleGORM, error) {
	...
	if len(query) == 0 {
		query = []string{"id", "name", "desired_status", "create_time", "schema_attr", "fn", "ebpf"}
	}
...
func (g *ModuleDao) ListByStatus(desiredStatus int) ([]ModuleGORM, error) {
	var moduleList []ModuleGORM
	result := g.Client.Engine.Where(&ModuleGORM{DesiredStatus: desiredStatus}).Order("create_time desc").Find(&moduleList)
	if result.Error != nil {
