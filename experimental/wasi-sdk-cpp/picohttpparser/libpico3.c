/**
 * Difference from libpico.c:
 *
 * libpico.c:  allocate input & output memory in wasm module;
 * libpico3.c: transfer input/output data via host created linear memories;
 *             prototyping purpose, not working yet, see README.md for why.
 *
 */
...
// Functions expected to be provided by the embedder.
extern struct http_event *get_input_buf();
extern struct parse_result *get_output_buf();
...
// the sandbox creator?
static struct http_event *input_buf = NULL;
static struct parse_result *output_buf = NULL;
...
int pico_parse_events(int num_reqs) {
  ...
  input_buf = get_input_buf();
  if (!input_buf) {
    printf("Input buffer uninitialized\n");
    return RET_MEM_ERR;
  }

  printf("Got input buffer %p\n", input_buf);

  output_buf = get_output_buf();
  if (!output_buf) {
    printf("Output buffer uninitialized\n");
    return RET_MEM_ERR;
  }

  printf("Got output buffer %p\n", output_buf);
