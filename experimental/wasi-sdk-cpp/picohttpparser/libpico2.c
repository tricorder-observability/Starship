 ...
 * libpico2.c: receive input data via externref;
...
// Call this function with input data. Parsed results will be stored in
// output_buf.
//
// Parameters:
...
// int pico_parse_events(int num_reqs, externref events) {
int pico_parse_events(int num_reqs,
                      char __attribute__((address_space(10))) * events) {
  // Reset output stats
  output_item_count = 0;
  ...
  struct http_event *input_buf = (struct http_event *)events;
