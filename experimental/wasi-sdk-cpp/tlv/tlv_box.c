...
static int tlv_box_putobject(tlv_box_t *box, int type, void *value,
                             int length) {
...
int tlv_box_get_size(tlv_box_t *box) { return box->m_serialized_bytes; }
...
int tlv_box_put_bytes(tlv_box_t *box, int type, unsigned char *value,
                      int length) {
...
int tlv_box_get_bytes(tlv_box_t *box, int type, unsigned char *value,
                      int *length) {
  value_t object;
  if (key_list_get(box->m_list, type, &object) != 0) {
    return -1;
...
int tlv_box_get_bytes_ptr(tlv_box_t *box, int type, unsigned char **value,
                          int *length) {
  value_t object;
  if (key_list_get(box->m_list, type, &object) != 0) {
    return -1;
